...
// Versi: 2.3 (Manajemen Slug & Logo URL)
...
function renderSchoolsTable() {
    ...
    if (AppStateAdmin.schools.length === 0) {
        tableBody.innerHTML = `<tr><td colspan="4" style="text-align: center; padding: 20px;">Belum ada sekolah terdaftar.</td></tr>`;
        return;
    }

    // Menambahkan kolom Slug & Logo ke tampilan tabel
    tableBody.innerHTML = AppStateAdmin.schools.map(school => `
        <tr>
            <td><strong>${school.nama_sekolah}</strong></td>
            <td><code>${school.slug || '-'}</code></td>
            <td><a href="${school.logo_url || '#'}" target="_blank" rel="noopener noreferrer">${school.logo_url ? 'Lihat Logo' : '-'}</a></td>
...
function renderUsersTable() {
    ...
    tableBody.innerHTML = AppStateAdmin.users.map(user => {
        const schoolName = user.sekolah?.nama_sekolah ? `<span style="color: var(--success-dark); font-weight:500;">${user.sekolah.nama_sekolah}</span>` : `<span style="color: var(--danger-color); font-style:italic;">Belum Tertaut</span>`;
        const actionButtons = user.role === 'super_admin'
            ...
            : `<button class="btn btn-sm btn-secondary" onclick="window.handleChangeUserSchool('${user.id}', '${user.email}')">Ubah</button>
...
function setupEventListeners() {
    // ==== PERBAIKAN DI SINI: Form Tambah Sekolah ====
    document.getElementById('formTambahSekolah')?.addEventListener('submit', async (e) => {
        ...
        if (!schoolName || !slug) {
            return showStatusMessage('Nama Sekolah dan Slug URL wajib diisi.', 'error');
        }
        ...
        const { error } = await supabase.from('sekolah').insert({
            ...
            slug: slug,
            logo_url: logoUrl || null // Kirim null jika kosong
        });
        showLoading(false);
    ...
    document.getElementById('formPengguna')?.addEventListener('submit', async (e) => {
        ...
        const { data, error } = await supabase.functions.invoke('quick-task', {
            body: { action: 'createUser', payload: { email, password, schoolId } }
        });
        showLoading(false);
...
window.handleDeleteSchool = async function(schoolId, schoolName) {
    ...
    showLoading(false);
    if (error) showStatusMessage(`Gagal menghapus: ${error.message}`, 'error');
    else {
...
window.handleDeleteUser = async function(userId, userEmail) {
    ...
    showLoading(false);
    if (error || (data && data.error)) showStatusMessage(`Gagal menghapus: ${error?.message || data?.error}`, 'error');
    else {
...
window.handleChangeUserSchool = async function(userId, userEmail) {
    ...
    showLoading(false);
    if (error) showStatusMessage(`Gagal update: ${error.message}`, 'error');
    else {
