 ...
 * =================================================================
 * @version 2.2 - Fixed Infinite Redirect Loop
 * @author Gemini AI Expert for User
 *
 * Catatan:
 * - [FIX v2.2] Menggunakan onAuthStateChange untuk memeriksa status login
 *   secara andal dan mencegah infinite redirect loop.
 */
...
/**
 * Titik masuk utama untuk halaman admin.
 * Menggunakan onAuthStateChange untuk menangani status otentikasi.
 */
function initSuperAdminPage() {
    supabase.auth.onAuthStateChange((event, session) => {
        if (event === 'INITIAL_SESSION' || event === 'SIGNED_IN') {
            if (!session) {
                // Jika tidak ada sesi sama sekali, paksa ke halaman login.
            ...
            if (isSuperAdmin) {
                // Pengguna adalah Super Admin, lanjutkan memuat halaman.
                document.getElementById('welcomeMessage').textContent = `Admin: ${session.user.email}`;
                loadAdminData();
                setupEventListeners();
            } else {
                // Pengguna login tapi bukan Super Admin, usir ke dashboard biasa.
                alert('AKSES DITOLAK! Halaman ini hanya untuk Super Administrator.');
                window.location.replace('dashboard.html');
            }
        } else if (event === 'SIGNED_OUT') {
            // Jika pengguna logout, paksa ke halaman login.
            window.location.replace('index.html');
        }
    });
}
...
function setupEventListeners() {
    ...
    document.getElementById('logoutButton')?.addEventListener('click', () => {
         if (confirm('Yakin ingin logout dari Panel Admin?')) {
            supabase.auth.signOut(); // onAuthStateChange akan menangani pengalihan
         }
    });
}
...
async function handleCreateUserSubmit(event) {
    ...
    if (!email || !password || !schoolId) return showStatusMessage('Harap isi semua field: Email, Password, dan Sekolah.', 'error');
    if (password.length < 6) return showStatusMessage('Password minimal 6 karakter.', 'error');
    showLoading(true);
    const { data, error } = await supabase.rpc('admin_create_user_for_school', { user_email: email, user_password: password, target_school_id: schoolId });
    showLoading(false);
...
async function handleChangeUserSchool(userId, userEmail) {
    ...
    try {
        ...
        let currentMetaData = {};
        try { if (currentMetaText) { currentMetaData = JSON.parse(currentMetaText); } } catch (e) { console.warn("Metadata rusak, akan ditimpa.", e); currentMetaData = {}; }
        if (typeof currentMetaData !== 'object' || currentMetaData === null) currentMetaData = {};
        currentMetaData.sekolah_id = selectedSchool.id;
        const { error: setError } = await supabase.rpc('admin_set_user_metadata', { target_user_id: userId, new_metadata: currentMetaData });
...
// Jalankan fungsi inisialisasi saat DOM siap.
document.addEventListener('DOMContentLoaded', initSuperAdminPage);
